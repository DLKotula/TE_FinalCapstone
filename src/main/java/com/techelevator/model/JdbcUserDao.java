...
public class JdbcUserDao implements UserDao {
    ...
    public User saveUser(String userName, String password, String role) {
        ...
        addToDefaultCollection(newUser);
        return newUser;

    }
    @Override
    public void addToDefaultCollection(User newUser){
        Collection collection = collectionDao.addDefaultCollection(newUser.getUsername()+ " collection", true, true, "https://www.vobss.com/wp-content/uploads/2021/11/spiderman-pointing-meme-vobss-9.jpg");
        Long collectionId = collection.getCollectionId();
        collectionDao.addDefaultCollectionToJoinTable(newUser.getId(), collectionId);
    }
    ...
    public  List<User> searchUsers(String search) {
        ...
        String customerSearchSql = "SELECT * FROM app_user WHERE user_name ILIKE ?";
        SqlRowSet results = jdbcTemplate.queryForRowSet(customerSearchSql, "%" + search + "%");
        while (results.next()) {
            matchingUsers.add(mapResultToUser(results));
        }
